...
Прикладные сообщения по-прежнему имеют строковый формат, но транспорт зависит
от клиента:

- web-клиент: Browser -> WebSocket -> Node.js bridge -> TLS -> C++ server;
- консольный/Qt клиент: прямое TCP-соединение с сервером.
...
TLS не меняет прикладной формат сообщений. Он только шифрует трафик между
bridge и сервером, чтобы текст нельзя было прочитать при перехвате пакетов.
